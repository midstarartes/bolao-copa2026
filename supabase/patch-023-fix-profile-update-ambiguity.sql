-- Patch 023: Corrige ambiguidade de colunas em app_update_current_user_profile
--
-- PROBLEMA: No UPDATE, expressões do tipo "ELSE nickname" eram ambíguas
--   entre a coluna da tabela app_users e a coluna da tabela de retorno
--   da própria função (returns table(nickname text, ...)).
--   O PostgreSQL lançava: "column reference 'nickname' is ambiguous".
--   Esse erro existia desde o patch-019, mas era engolido silenciosamente
--   pelo catch do JS. O patch-022 expôs o erro ao mostrar mensagens ao usuário.
--
-- CORREÇÃO: Usa v_user.<campo> nos ELSE — referência explícita ao record,
--   sem nenhuma ambiguidade com as colunas da tabela de retorno.
...
begin
  ...
  -- Usa v_user.<campo> nos ELSE para evitar ambiguidade com as colunas
  -- da tabela de retorno (returns table também declara "nickname", etc.)
  update public.app_users
  set
    nickname      = case
                      when p_nickname is not null and trim(p_nickname) <> ''
                      then trim(p_nickname)
                      else v_user.nickname
                    end,
    password_hash = case
                      when p_new_password is not null and trim(p_new_password) <> ''
                      then public.app_hash_password(p_new_password)
                      else v_user.password_hash
                    end,
    avatar_type   = coalesce(p_avatar_type,  v_user.avatar_type),
    avatar_value  = case
                      when p_avatar_type is not null then p_avatar_value
                      else v_user.avatar_value
                    end
  where id = v_user.id
